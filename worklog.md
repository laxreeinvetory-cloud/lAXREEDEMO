...
---
Task ID: MONTHLY-REPORT
Agent: monthly-report-admin (full-stack)
Task: Build a Monthly Report system for the LaxRee Amenities admin panel — Excel (.xlsx) + print-to-PDF (HTML) export, admin UI, and sidebar nav item.

Pre-work:
- Read /home/z/my-project/worklog.md (full history; most recent: HOMEPAGE-IMAGE-EDIT). Confirmed pre-existing infra mismatch: prisma/schema.prisma declares `provider = "postgresql"` while `.env` has `DATABASE_URL=file:/home/z/my-project/db/custom.db` (SQLite). DB reads on this box throw "URL must start with postgresql://". All my DB calls are wrapped in try/catch with static-fallback (blog + products) or empty (leads) so the report still renders cleanly.
- Confirmed `exceljs@^4.4.0` is already in package.json — no new installs needed.
- Read existing patterns: src/app/api/admin/stats/route.ts (stats shape), src/app/api/admin/leads/route.ts (lead fields), src/app/api/admin/blog/route.ts (DB → static fallback pattern), src/app/api/admin/products/route.ts (DB → static fallback pattern), src/app/api/admin/cms/route.ts (SiteContent.findUnique by key), src/app/admin/blog/page.tsx + src/app/admin/leads/page.tsx (admin page styling: glass-on-charcoal, brass accents, font-display/font-mono/font-body).
- Read src/lib/admin/admin-shell.tsx — flat navItems array grouped by comments (// OVERVIEW / // CRM / // CATALOGUE / etc.); brand tokens: bg-charcoal, text-ivory, text-sand, text-brass.
- Read prisma/schema.prisma — Lead { source, status, category, createdAt, name, phone, email, hotel }, BlogPost { published, createdAt, date, author, category }, Product { featured, published, category, model, name, price, createdAt }, SiteContent { key, value }.
- Lucide v0.525.0 does NOT export `FileBar`. Used `FileBarChart` (file-with-bar-chart icon) instead — semantically equivalent and visually identical for the Reports nav item.

Files created:
1. src/app/api/admin/report/route.ts (NEW — 1306 lines)
   - `export const runtime = "nodejs"` + `export const dynamic = "force-dynamic"`.
   - GET handler reads `?format=xlsx|pdf|html` (default xlsx) and optional `?month=YYYY-MM` (default previous month).
   - `gatherReportData(monthParam)` collects everything in one pass:
     • Leads from db.lead.findMany({ where: { createdAt: { gte: monthStart, lt: monthEnd } } }) — try/catch → empty array on DB error.
     • Blog posts from db.blogPost.findMany() → falls back to getStaticBlogPosts() when DB empty/unreachable.
     • Products from db.product.findMany() → falls back to getStaticProducts().
     • SiteContent keys "client-logos" + "images" → counts (array length or object key count); try/catch → 0.
     • Aggregations: leadsBySource, leadsByStatus, leadsByDay (one row per day in the month), topCategories (top 5 by request count).
   - `buildExcel(data)` returns ExcelJS buffer with 7 sheets:
     • Summary — charcoal title row + brass subtitle + 8 KPI cards in 2-col layout + Leads-by-Source / Leads-by-Status / Top-Categories pivots.
     • Leads — all leads (Date, Name, Phone, Email, Hotel, Category, Source, Status) with frozen header row.
     • Leads by Source — pivot: Source / Leads / % of Total.
     • Leads by Status — pivot: Status / Leads / % of Total.
     • Leads by Day — Date / Day / Leads for every day of the month (for charting).
     • Blog Posts — Date / Title / Category / Author / Read Time / Published / Slug.
     • Products — Model / Name / Category / Price / Featured / Published.
   - Excel styling: title row = charcoal bg + white bold 18px; subtitle row = charcoal bg + sand italic; header rows = brass (#C6A15B) bg + white bold; alternating data rows = ivoryAlt (#FBF8F1); thin borderLight bottom border per row; column widths preset per sheet; frozen panes on data sheets.
   - `buildHtml(data)` returns a self-contained HTML document (no external assets) with:
     • Sticky toolbar with `Print / Save as PDF` button (calls `window.print()`).
     • Charcoal gradient header with LaxRee logo mark, brand name, "Monthly Performance Report" title, month line, generated-on/period/report-id meta.
     • 8 KPI cards in a 4-col grid (brass + emerald accents).
     • Two-col Leads-by-Source + Leads-by-Status tables with share %.
     • Daily Lead Activity bar chart (pure CSS bars, no JS lib).
     • Top 5 Categories table.
     • Recent Leads table (top 20) with status pills.
     • Blog Posts table (top 10) with Published/Draft badges.
     • Products by Category table.
     • Website Stats: top-pages table (page-views shown as "N/A — connect GA") + client-logos / image-overrides KPI cards.
     • Footer with brand + generated timestamp.
     • `@media print` block: A4 page size, 14mm margins, hide toolbar, ensure colors print, page-break-inside: avoid on sections.
   - Response headers:
     • xlsx → `Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet` + `Content-Disposition: attachment; filename="laxree-report-{month}.xlsx"`.
     • pdf/html → `Content-Type: text/html; charset=utf-8` + `Content-Disposition: attachment; filename="laxree-report-{month}.html"`.
   - 500 → JSON error response with message + error string.

2. src/app/admin/reports/page.tsx (NEW — 493 lines, "use client")
   - Page header: brass icon tile + "Admin · Reporting" eyebrow + "Monthly Reports" h1 + subtitle.
   - "Last generated" card (top-right on desktop) — persisted to localStorage on each successful download.
   - Month selector card: dropdown of last 6 months (YYYY-MM values, "Month Year" labels) + "Reporting Period" brass chip showing the selected label.
   - Two big download buttons in a 2-col grid:
     • "Download Excel" — brass gradient card with FileSpreadsheet icon, .xlsx label, Loader2 spinner while downloading.
     • "Download PDF (HTML)" — emerald gradient card with FileText icon, .html · print-to-pdf label, Loader2 spinner while downloading.
   - Both buttons: fetch the API → blob → create object URL → trigger `<a download>` click → update lastGenerated → show success toast. On error: red toast with the failure message. Toast auto-dismisses after 4s.
   - "Live Snapshot" section: refresh button + 4 KPI cards (Total Leads, New Leads, Blog Posts, Published) sourced from /api/admin/stats. Loading state shows Loader2 spinner inside a glass card.
   - Leads-by-Source breakdown panel: top 5 sources with name + count + % and a brass-gradient progress bar (max-h-72 overflow-y-auto scrollbar-thin).
   - Recent Leads panel: top 5 from /api/admin/stats with avatar, name, source · date, status pill.
   - Footer note explaining data sources + GA limitation.
   - Toast notification (bottom-right, glass-on-charcoal, emerald or red border).
   - KPI cards use border-l-4 brass/emerald accent + font-display 3xl value + font-mono uppercase label.

3. src/lib/admin/admin-shell.tsx (MODIFIED)
   - Added `FileBarChart` to lucide-react imports (FileBar is not exported by lucide-react@0.525.0).
   - Added `{ label: "Reports", href: "/admin/reports", icon: FileBarChart }` to navItems immediately after the "Dashboard" entry in the OVERVIEW section.

4. src/app/admin/images/page.tsx (MODIFIED — 1-line lint fix)
   - Line 105: `let current: any = getData.value || {};` → `const current: any = getData.value || {};` (variable is never reassigned). Pre-existing prefer-const error that was blocking `bun run lint` from passing; not introduced by this task but had to fix to satisfy the "0 lint errors" verification gate.

Verification:
- `npx tsc --noEmit`: EXIT 0 — zero TypeScript errors.
- `bun run lint`: 0 errors, 32 warnings — all pre-existing `@next/next/no-img-element` warnings in OTHER files (catalogue, products, components/site/*, admin-shell, hero-stage, db.ts unused eslint-disable). ZERO new warnings introduced by this task.
- Dev server (Next.js 16.1.3 Turbopack) started on :3000. Routes verified with curl:
  • `GET /admin/reports` → HTTP 200, 40,499 bytes — valid HTML (admin shell renders loading spinner server-side, then hydrates to the full reports UI client-side after auth check).
  • `GET /api/admin/report?format=xlsx` → HTTP 200, 23,141 bytes — `file` reports "Microsoft Excel 2007+". Parsed with ExcelJS: 7 sheets present (Summary, Leads, Leads by Source, Leads by Status, Leads by Day, Blog Posts, Products). Summary A1 cell = "LaxRee Amenities — Monthly Report", 21 rows.
  • `GET /api/admin/report?format=pdf` → HTTP 200, 26,565 bytes — `file` reports "HTML document, Unicode text, UTF-8". Contains "Monthly Performance Report" title (1×), "Print / Save as PDF" button (1×), 14 KPI card class instances.
- DB errors in dev.log (`Error validating datasource db: the URL must start with the protocol postgresql://`) are EXPECTED — the prisma schema/.env mismatch is pre-existing per the worklog. My route wraps every DB call in try/catch and falls back to static data (blog + products) or empty arrays (leads), so the report renders cleanly. Once the schema is switched to sqlite + `bun run db:push` is run, the report will pull live lead/blog/product counts automatically.
- The previous-month default works: dev server time reports "July 2026" in the HTML title (current month = August 2026, default = previous month = July 2026).

Stage Summary:
- New endpoints: `/api/admin/report?format=xlsx` (Excel download), `/api/admin/report?format=pdf` (print-to-PDF HTML download), `/api/admin/report?format=html` (alias for pdf).
- New admin page: `/admin/reports` with month selector, dual download buttons, live KPI preview, recent-leads panel, leads-by-source breakdown, and last-generated timestamp.
- New nav item in admin sidebar: "Reports" (FileBarChart icon) in OVERVIEW section, right after Dashboard.
