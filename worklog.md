...
=======
Task ID: FAQ-SECTION
Agent: full-stack-developer (FAQ section)
Task: Build FAQ page + SEO schema + admin editor

Work Log:
- Read worklog + reference files: /admin/seo/page.tsx (admin editor style), /api/admin/settings/route.ts (key-value SiteContent pattern), /app/globals.css (charcoal #12100D / ivory #F7F3E8 / brass #C6A15B / emerald #1E4638 / sand design tokens; glass-on-charcoal utility; eyebrow / data-label / pill classes), /lib/laxree/site-data.ts (NAV_LINKS array + SITE constant), /components/site/navbar.tsx (consumes NAV_LINKS from site-data.ts), /app/layout.tsx (existing JSON-LD structured-data pattern), /app/admin/blog/page.tsx (admin list+modal style reference), /app/career/page.tsx (accordion + page primitives reference), /components/site/page-primitives.tsx (PageHero, FadeIn, GlassCard, SectionHeading, PageCTA), prisma/schema.prisma (confirmed SiteContent { id, key, value, updatedAt } already exists — did NOT touch schema).
- Deliverable 1 — FAQ API (`src/app/api/admin/faq/route.ts`):
  - `export const runtime = "nodejs"`, `import { db } from "@/lib/db"`. All FAQ items persisted as a single JSON array under SiteContent key `faq`.
  - Exports `FaqItem` type `{ id, question, answer, category, sortOrder, published }` + `DEFAULT_FAQS` (the 10 default FAQs from spec — Products, Logistics, Ordering×2, Quality×2, Partnership, General×2, Services).
  - GET: reads SiteContent row `faq`, parses JSON, validates each item with a type guard (falls back to DEFAULTS if missing/corrupt/unparseable). Sorted by `sortOrder asc`. Supports `?published=true` to filter out unpublished items (used by the public /faq page). try/catch wraps the DB read → on error returns DEFAULTS (so the public page never 500s).
  - POST: validates `question`/`answer`/`category` required. Generates id as `faq-{timestamp}-{rand6}`. Defaults `sortOrder` to `items.length`, `published` to `true`. Reads full array → push → write back.
  - PATCH: takes `{ id, ...fields }`, finds item, updates only the provided fields (preserves the rest), writes back. 404 if not found.
  - DELETE: `?id=...` query, filters out item, writes back. 404 if not found.
  - All handlers wrapped in try/catch with `console.error("[ADMIN FAQ ... ERROR]", err)` + 500 response. `writeFaqs` uses `db.siteContent.upsert`.
- Deliverable 2 — Public FAQ page (`src/app/faq/page.tsx` + `src/app/faq/layout.tsx`):
  - `"use client"` page. Layout: metadata title "FAQ — Frequently Asked Questions" + description.
  - PageHero (charcoal, eyebrow "FAQ", title "Frequently Asked Questions", subtitle "Everything you need about LaxRee Amenities products and services." + breadcrumb Home › FAQ).
  - Search bar (glass-on-charcoal pill, Search icon left, X clear button right when query non-empty) — filters by question + answer + category text.
  - Category filter pills (All, Products, Ordering, Logistics, Quality, Partnership, Services, General) — active = pill-brass, inactive = pill-ghost-ivory with brass hover.
  - Results meta line ("N questions in <category> matching "<query>").
  - Accordion list: each item is a glass-on-charcoal card with brass-bordered category badge (pill-style), question (font-display ivory), ChevronDown icon that rotates 180° when open. Click toggles open state; Framer Motion `AnimatePresence` animates height auto↔0 with opacity for smooth expand/collapse. When open, shows a hairline-brass divider then the answer (font-body sand).
  - Empty state: HelpCircle icon + "No matching questions found" + helpful copy (uses GlassCard).
  - Loading state: brass spinner.
  - "Still have questions?" CTA section (emerald background): 3-column grid of contact cards — Phone (tel: link, SITE.phoneDisplay), WhatsApp (wa.me link, SITE.whatsapp), Email (mailto:, SITE.email) — each with brass icon circle. Below: "Visit Contact Page" link to /contact-us.
  - SEO: builds JSON-LD `FAQPage` schema from the (filtered) FAQ data — `mainEntity` array of `{ "@type": "Question", name, acceptedAnswer: { "@type": "Answer", text } }` — and injects via `<script type="application/ld+json" dangerouslySetInnerHTML>` for Google rich snippets. Schema updates reactively when filters/search change.
  - Fetches `/api/admin/faq?published=true` on mount with `cache: "no-store"` + cancellation flag for unmount safety.
  - Lucide icons used: Search, ChevronDown, Phone, Mail, MessageCircle, HelpCircle, X.
- Deliverable 3 — Admin FAQ editor (`src/app/admin/faq/page.tsx`):
  - `"use client"` page rendering inside AdminShell. Header: "FAQ Management" + subtitle showing count + SiteContent key. "Add FAQ" brass pill button.
  - Filter bar (glass-on-charcoal card): Search input (left, Search icon) + Category dropdown (All / Products / Ordering / Logistics / Quality / Partnership / Services / General).
  - FAQ table (glass-on-charcoal card): desktop 5-col grid (Question | Category | Order | Status | Actions). Question col shows question (font-display ivory, line-clamp-2) + answer preview (font-body sand, line-clamp-1). Category = brass pill. Order = `#sortOrder` mono. Status = inline toggle button (Eye/EyeOff + "Live"/"Hidden" — emerald when published, sand when hidden) — click calls PATCH to toggle published. Actions = Pencil (edit) + Trash2 (delete with confirm()).
  - Empty state: HelpCircle icon + "No FAQ items found" + "Create First FAQ" button.
  - Editor modal (add/edit): glass-on-charcoal rounded-2xl, max-w-2xl, max-h-85vh scroll. Inputs use shared `rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-ivory ... focus:border-brass focus:outline-none` style (matches /admin/blog + /admin/seo). Fields: Question* (text, col-span-2), Answer* (textarea 5 rows, char counter, col-span-2), Category (select of 7 categories), Sort Order (number input with "Lower numbers appear first" hint), Published checkbox (with "Published (visible on /faq)" label). Save button disabled until question + answer non-empty. Save: POST (new) or PATCH (edit) to /api/admin/faq then refetch + close modal.
  - Lucide icons: Plus, Pencil, Trash2, Search, Eye, EyeOff, X, HelpCircle.
- Deliverable 4 — FAQ nav link:
  - Task said "edit navbar.tsx and find the NAV_LINKS array (or equivalent nav items)". navbar.tsx imports NAV_LINKS from `@/lib/laxree/site-data.ts` (single source of truth used by both desktop nav and mobile drawer). Edited `src/lib/laxree/site-data.ts` NAV_LINKS array to add `{ label: "FAQ", href: "/faq" }` between "Career" and "Contact Us". This automatically propagates to both the desktop nav and the mobile full-screen drawer in navbar.tsx — no other file needs touching.
- Constraints respected: did NOT touch prisma/schema.prisma (SiteContent already exists), did NOT touch src/lib/admin/admin-shell.tsx (orchestrator will wire admin nav), did NOT touch src/app/admin/content/page.tsx (orchestrator will wire links). Used only deps already in package.json (next, react, lucide-react, framer-motion, prisma). 
- Ran `cd /home/z/my-project && bun run lint 2>&1 | tail -5` → `✖ 33 problems (0 errors, 33 warnings)` — all 33 are pre-existing `<img>` warnings in OTHER files; zero new errors, zero new warnings in any FAQ file. Lint is clean.

Stage Summary:
- Files created (4):
  1. `/home/z/my-project/src/app/api/admin/faq/route.ts` — FAQ CRUD API over SiteContent JSON (`faq` key). GET (with `?published=true` filter + default fallbacks), POST, PATCH, DELETE. Returns 10 default FAQs when DB empty/corrupt.
  2. `/home/z/my-project/src/app/faq/layout.tsx` — metadata (title + description).
  3. `/home/z/my-project/src/app/faq/page.tsx` — public FAQ page: PageHero + search bar + category pills + animated accordion (Framer Motion) + JSON-LD FAQPage schema for Google rich snippets + "Still have questions?" emerald CTA with Phone/WhatsApp/Email contact cards.
  4. `/home/z/my-project/src/app/admin/faq/page.tsx` — admin FAQ editor: searchable/filterable table + add/edit modal (question, answer, category, sortOrder, published) + inline publish toggle + delete with confirm.
- Files modified (1):
  - `/home/z/my-project/src/lib/laxree/site-data.ts` — added `{ label: "FAQ", href: "/faq" }` to NAV_LINKS between "Career" and "Contact Us" (navbar.tsx imports NAV_LINKS from here, so this propagates to both desktop and mobile nav).
- Lint: clean — 0 errors (33 pre-existing `<img>` warnings, none in FAQ files).
- SEO: FAQ page emits JSON-LD `FAQPage` schema for Google rich snippets (eligible for expandable FAQ display in search results). Schema is built client-side from the actual published FAQ data so it stays in sync with admin edits.
- Public site link verified: navbar now shows FAQ between Career and Contact Us on both desktop and mobile drawer.
