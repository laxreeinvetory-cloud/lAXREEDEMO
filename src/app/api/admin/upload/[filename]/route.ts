...
/* Serve an image stored as base64 data URL in SiteContent.
   Key format: `image:<filename>`. Returns raw bytes with long cache. */
export async function GET(
  ...
  try {
    ...
    if (!row || !row.value) {
      // Return 1x1 transparent pixel instead of 404 broken image
