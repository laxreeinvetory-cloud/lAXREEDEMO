...
# Load .env to override any stale system DATABASE_URL (e.g. local SQLite)
if [ -f "$PROJECT_DIR/.env" ]; then
        echo "[ENV] Loading .env to override system environment variables..."
        set -a
        # shellcheck disable=SC1090
        . "$PROJECT_DIR/.env"
        set +a
        echo "[ENV] DATABASE_URL is now: ${DATABASE_URL:0:40}..."
fi
